...
public class SectionDAO {
    ...
    public boolean updateSection(SectionModel section) {
        String sql = "UPDATE Sections SET section_title = ?, is_hidden = ? WHERE textbook_id = ? AND chapter_id = ? AND section_id = ?";
        try (Connection conn = DatabaseConnection.getInstance().getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {

            pstmt.setString(1, section.getTitle());
            pstmt.setBoolean(2, section.isHidden());
            pstmt.setString(3, section.getTextbookId());
            pstmt.setString(4, section.getChapterId());
            pstmt.setString(5, section.getSectionId());
    ...
    // Delete a section
    public boolean deleteSection(String textbookId, String chapterId, String sectionId) {
        String sql = "DELETE FROM Sections WHERE textbook_id = ? AND chapter_id = ? AND section_id = ?";
        try (Connection conn = DatabaseConnection.getInstance().getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
